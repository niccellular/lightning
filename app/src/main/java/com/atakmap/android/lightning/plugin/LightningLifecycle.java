...
public class LightningLifecycle implements Lifecycle {

    private final Context pluginContext;
    private final Collection<MapComponent> overlays;
    private MapView mapView;

    private final static String TAG = "LightningLifecycle";

    public LightningLifecycle(Context ctx) {
    ...
    public void onCreate(final Activity arg0,
        ...
        LightningLifecycle.this.overlays
                .add(new LightningMapComponent());

        // create components
        Iterator<MapComponent> iter = LightningLifecycle.this.overlays
                .iterator();
        MapComponent c;
        while (iter.hasNext()) {
            c = iter.next();
            try {
                c.onCreate(LightningLifecycle.this.pluginContext,
                        arg0.getIntent(),
                        LightningLifecycle.this.mapView);
            } catch (Exception e) {
                Log.w(TAG,
